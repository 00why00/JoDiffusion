...
def decode_seg(array_seg_t, cmap=None):
    # we will decode the semseg map with a fixed color map
    if cmap is None:
        cmap = color_map()
    seg_t = array_seg_t.astype(np.uint8)
    array_semseg = np.zeros((seg_t.shape[0], seg_t.shape[1]), dtype=np.uint8)
    for i, color in enumerate(cmap):
        equal = np.all(seg_t == color, axis=-1)
        array_semseg[equal] = i
    return array_semseg
